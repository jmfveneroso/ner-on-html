import np
import re
from models.util import load_raw_dataset
...
class HiddenMarkov:
  ...
  def train_features(self, X, Y, which_features=[]):
    ...
    for i in range(len(Y)):
      for j in range(50):
        ...
        # f = X[i][j][2:1+self.num_all_features]
  ...
  def train_transitions(self, X, Y):
    ...
    #   print(self.idx_to_states(s), self.transition_mat[s,:])
    ...
    for i in range(len(X)):
      ...
      for s in range(self.num_states):
        for l in range(self.num_labels):
          ...
          # if is_punctuation(f[0]) and l != 2:
          #   state_probs[idx,0] = 0
          # elif not is_punctuation(f[0]) and l == 2:
          #   state_probs[idx,0] = 0
